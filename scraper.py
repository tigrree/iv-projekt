...
def scrape_bger():
    ...
    try:
        ...
        # Die letzten 14-20 Tage der Index-Seite
        ...
        for datum, link in date_links:
            ...
            # Wir durchsuchen die Tabelle der Tagesliste (wie im Screenshot markiert)
            rows = day_soup.find_all('tr')
            for row in rows:
                cols = row.find_all('td')
                # Eine gültige Zeile hat Geschäftsnummer (2. Spalte) und Sachgebiet (3. Spalte)
                if len(cols) >= 3:
                    az_cell = cols[1]
                    sachgebiet_cell = cols[2]
                    
                    az = az_cell.get_text().strip()
                    sachgebiet_text = sachgebiet_cell.get_text().strip()
                    
                    # 1. Filter: Nur IV-relevante Begriffe im SACHGEBIET (siehe Screenshot)
                    if any(kw.lower() in sachgebiet_text.lower() for kw in IV_KEYWORDS):
                        link_tag = az_cell.find('a', href=True)
                        if link_tag:
                            iv_urteil_an_diesem_tag = True
                            full_link = link_tag['href'] if link_tag['href'].startswith("http") else domain + link_tag['href']
                            
                            # Nur wenn relevant, laden wir das Urteil für die KI
                            if az in archiv:
                                zusammenfassung = archiv[az]
                            elif ai_counter < limit:
                                print(f"Analysiere IV-Urteil aus Sachgebiet: {az}")
