...
def summarize_with_ai(urteil_text):
    ...
    # Dein neues Wunschformat mit \n und fettgedruckten Titeln
    PROMPT_TEXT = """
...
def scrape_bger():
    print(f"--- Starte Spezial-Run (Layout-Update) für: {ZIEL_DATUM} ---")
    ...
    try:
        ...
        for i in range(len(rows)):
            ...
            # --- VORSCHAU-LOGIK ---
            # Wir suchen gezielt nach dem Text unter dem Aktenzeichen/Sachgebiet
            # row.find_all(string=True) gibt uns alle Textstücke in der Zeile
            text_parts = [t.strip() for t in row.find_all(string=True) if t.strip()]
            
            # In der BGer Tabelle ist der 4. Textteil meist das spezifische Rechtsgebiet (2. Absatz)
            vorschau_text = ""
            if len(text_parts) >= 4:
                # Wir filtern Datum und Aktenzeichen raus, um den Text darunter zu finden
                vorschau_text = text_parts[3] 
            
            ctx = (row.get_text() + " " + (rows[i+1].get_text() if i+1 < len(rows) else "")).lower()
            if any(key in ctx for key in KEYWORDS):
                print(f"Analysiere: {az} mit Vorschau: {vorschau_text}")
        ...
        # Sicherstellen, dass nur 14 Tage bleiben (Auto-Delete)
        ...
        print("Erfolg! Der 17.02. wurde mit dem neuen Layout-Format gespeichert.")
